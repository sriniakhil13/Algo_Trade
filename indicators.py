...
def SuperTrend(df, period, multiplier, ohlc=['Open', 'High', 'Low', 'Close']):
    ...
    df['basic_ub'] = (pd.to_numeric(df[ohlc[1]]) + pd.to_numeric(df[ohlc[2]])) / 2 + multiplier * df[atr]
    df['basic_lb'] = (pd.to_numeric(df[ohlc[1]]) + pd.to_numeric(df[ohlc[2]])) / 2 - multiplier * df[atr]
    # print(df['basic_ub'])
